...
function [motorModelNew] = MMM_skew_allTemperatures(motorModelNew,motorModelOld)
...
disp('Skewing maps at different PM temperature...')
for ii=1:length(motorModelNew.PMtempModels.tempVectPM)
    ...
    tmp = MMM_skew(tmp,motorModelNew.tmpSkew,[]);
    ...
    disp([' - map ' int2str(ii) ' of ' int2str(length(motorModelNew.PMtempModels.tempVectPM)) ' skewed'])
end
disp('All maps skewed!')
