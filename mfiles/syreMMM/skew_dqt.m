    ...
    for qq=1:qS
        ...
        for ss=1:length(alfa_k)
            ...
            if strcmp(motorType,'SR')
                ...
                FdSlice(real(Idq)<IdMin) = -fInt.Fd(-IdSlice(real(Idq)<IdMin),IqSlice(real(Idq)<IdMin),thSliceInv(real(Idq)<IdMin));
                FqSlice(real(Idq)<IdMin) = +fInt.Fq(-IdSlice(real(Idq)<IdMin),IqSlice(real(Idq)<IdMin),thSliceInv(real(Idq)<IdMin));
                TSlice(real(Idq)<IdMin)  = -fInt.T(-IdSlice(real(Idq)<IdMin),IqSlice(real(Idq)<IdMin),thSliceInv(real(Idq)<IdMin));
...
Fd   = griddedInterpolant(Id,Iq,mean(dqtMap.data.Fd,3),'linear','linear');
Fq   = griddedInterpolant(Id,Iq,mean(dqtMap.data.Fq,3),'linear','linear');
T    = griddedInterpolant(Id,Iq,mean(dqtMap.data.T,3),'linear','linear');
