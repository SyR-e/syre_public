...
TwMap.T     = Tmap;             % torque reference [Nm]
TwMap.Tout  = nan(size(nmap));  % torque produced (for operating limits) [Nm]
...
%         if strcmp(TwData.SkinEffectMethod,'LUT')
%             kAC = interp1(motorModel.skinEffect.f,motorModel.skinEffect.k,FreqElet);
%         else
%             kAC = polyval(motorModel.skinEffect.p,FreqElet);
%         end
    else
        kAC = 1;
    end
...
%         pause(0.01);
        ...
        TwMap.Eo(ii)    = interp2(Id,Iq,abs(Vind),id,iq);
        TwMap.Ife(ii)   = interp2(Id,Iq,abs(Ife),id,iq);
        if strcmp(motorType,'IM')
...
% resta da fare plot e salvataggio e controllare i profili minimo e massimo
...
[c,h] = contour(Id,Iq,T,'k','DisplayName','$T$ [Nm]');
clabel(c,h)
