...
for i = 1:length(h_1_vector)
    ...
    s_t1_h = s_t1_h(i,:)./10^6;    % Pa to MPa

    name = sprintf('$h_{sl}$ = %.2g mm',par.h_1_loop);
    j = j+1;
    if par.h_1_loop == par.h_1
    h(j) = plot(n_arr, s_t1_h,"LineStyle","-",'DisplayName',name,'LineWidth',2);
    else
    h(j) = plot(n_arr, s_t1_h,"LineStyle","-",'DisplayName',name,'LineWidth',1);
    end
    legend(h)
    hold on;
...
for i = 1:length(du_12_vector)
    ...
    for k = 1:1:size(w_arr,2)
        ...
        s_t1_p(i,k)  = p_12 * (par.r_1av_loop / par.h_1_loop) + w^2 * par.rho_1 * par.r_1av_loop^2;
    end

    s_t1_p = s_t1_p(i,:)./10^6;    % Pa to MPa

    if i == 1
    j = j+1;
    h(j) = plot(n_arr, s_t1_p,'LineStyle','-',"Color",'k','DisplayName','$du_{12}$','LineWidth',0.5);
    else
    ...
    j = j+1;
    h(j) = plot(n_arr, sleeve_max_stress,'LineStyle','--','DisplayName','$\sigma_{max}$','Color','r');
