...
b = 0.5 * vecnorm(AB, 2, 2); % replaced magn(AB,2) with vecnorm(AB, 2, 2)

% find triangle height
h = b ./ tan(angle ./ 2);

% find circle centre
% centre = M + bsxfun(@times, h, unit(V,2)); % unit function is undefined, replace with normalization
centre = M + bsxfun(@times, h, V ./ vecnorm(V, 2, 2)); % normalized V
