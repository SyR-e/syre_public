...
load motorModel;
Slx_name = [motorModel.data.motorName '_ctrl'];
...
p       = motorModel.data.p;
J       = motorModel.data.J;
accel   = 30000; % rpm/s -- for speed control only
% Converter
...
%% ----------------Flux Initialization and Axes Convention------------------------%
...
switch(Quad_Maps)
    case 0                  % Synchronous Reluctance Motor
        Fm = 0;
        InitIntg_d = 0.0;
        InitIntg_q = 0.0;
    case 1                  % PM-assisted Synchronous Reluctance Motor (PM on -q)
        Fm = abs(interp2(Idd,Iqq,Fqq,0,0));
        InitIntg_d = Fm*sin(th0);
        InitIntg_q = -Fm*cos(th0);
    case 2                  % IPM Motor (PM on d)
