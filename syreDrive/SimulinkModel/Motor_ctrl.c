...
/*this routine compute the outputs of S-Function block */
{
	...
	switch(State){
        ...
        case ERROR:
			...
			// sp_par.kp   = OMEGA_BW*J;
			// sp_par.ki   = OMEGA_BW*sp_par.kp*0.1*Ts;
			sp_par.ki   = OMEGA_BW*OMEGA_BW*J*Ts;
			sp_par.kp   = OMEGA_BW*J*2.0f*1.0f;
			// Current vector control calibration
			id_par.kp   = OMEGA_BI*ld;
			id_par.ki   = id_par.kp*OMEGA_BI*0.1*Ts;
			iq_par.kp   = OMEGA_BI*lq;
			iq_par.ki   = iq_par.kp*OMEGA_BI*0.1*Ts;
			// DFVC calibration
			lambda_par.kp = OMEGA_BI;
			lambda_par.ki = lambda_par.kp*OMEGA_BI*0.1f*Ts;
			iqs_par.kp = OMEGA_BI*lq;
			iqs_par.ki = iqs_par.kp*OMEGA_BI*0.1f*Ts;
			// encoder PLL
			PLL_mec_par.kp   = 2*OMEGA_PLL;
			PLL_mec_par.ki   = pow(OMEGA_PLL,2)*Ts;
			PLL_mec_par.lim  = RPM2RAD*nmax*2.0f;
			PLL_mec_var.ffw	 = 0.0f;

			// Sensorless control calibration
			if(SS_on){				
				switch (inj_waveform) {
					case 0: // sinusoidal 
				...
				PLL_par.lim  = RPM2RAD * nmax * PP;
			}
				
            if(Go) State = READY;	// Wait for Go state
        ...
        case START:
			...
			switch (Ctrl_type){
				...
				case 2: // TorqueControl
					...
					// DFVC flux reference
                    ramp(T_ext,50000*Ts,&T_ext_ramp);
                    ReadLut(&F_REF[0], fabs(T_ext_ramp), TMAX, TMIN, DT, INV_DT, &lambda_MTPA);
				break;
				...
				case 3: // SpeedControl
					...
					// DFVC flux reference
                    ramp(T_ext,150000*Ts,&T_ext_ramp);
                    ReadLut(&F_REF[0], fabs(T_ext_ramp), TMAX, TMIN, DT, INV_DT, &lambda_MTPA);
				break;
			}
			...
			if(SS_on){ //------SENSORLESS POSITION ESTIMATION-----------------//
				pos_err_LS = 0;
				pos_err_HS = 0;

				//--------------------HF demodulation-------------------------//
				if (fabsf(omega_elt) < KOBS + 1.2*OMEGA_G) {
					switch (dem) {
							case 0: // current dem
								switch(inj_waveform) {
				...
				switch (HS_ctrl) {
					...
					case APP:
						if (fabsf(omega_elt) > KOBS - 2*OMEGA_G) {
				...
				if (fabsf(omega_elt) < KOBS - OMEGA_G)
					f_omega = 0;
				else if (fabsf(omega_elt) > KOBS + OMEGA_G)
					f_omega = 1;	
				else
					f_omega = (OMEGA_G+fabsf(omega_elt)-KOBS)/(2*OMEGA_G);
            ...
            switch(Ctrl_strategy){
                case FOC: //-------FIELD ORIENTED CONTROL----------------//
                    
			    //----------------Feed Forward---------------------------//
                id_var.ffw = 1.0*(RS*isdq.d - omega_elt*lambda_dq.q);
                iq_var.ffw = 1.0*(RS*isdq.q + omega_elt*lambda_dq.d);

                //------------------Current Loops------------------------//
                Current_loop(vdc, Imax, isdq_ref, isdq ,&id_par, &id_var, &iq_par, &iq_var, &vsdq_ref);
                                    
                _invrot(vsdq_ref, SinCos_elt, vsab_ref);
            
                break;

                case DFVC: //------DIRECT FLUX VECTOR CONTROL------------//    

                lambda_ref = lambda_MTPA;
                
                //-------------------Flux Limitation----------------------//                  
                if(fabs(omega_elt)>0.1)   
                    Flux_Lim = 0.9f*(vdc*SQRT1OVER3-RS*idsqs.q*sgn(omega_elt))/fabs(omega_elt);
     
                if(Flux_Lim>lambda_MTPA_max*1.2)
                    Flux_Lim=lambda_MTPA_max*1.2;
                if(lambda_ref>Flux_Lim)         lambda_ref = Flux_Lim;
				...
				if(idsqs.d<Imax) {
				    tmp1=sqrt(pow(Imax,2)-pow(idsqs.d,2));
				}
                    else {
				        tmp1= 0.0f;
                    }

                PIRegAsy(&delta_par,&delta_var,tmp1,0);
                i_MTPV = delta_var.out;               
                   
                //-----------------Current Limitation--------------------//
                iqs_ref = T_ext_ramp/(1.5*PP*lambda_ref);
                
                iqs_Lim = tmp1-i_MTPV;
                if(fabs(iqs_ref) >iqs_Lim)
                    iqs_ref = iqs_Lim*sgn(T_ext_ramp);
                
                //---------------------Feed-Forward-----------------------//
                lambda_var.ffw = RS*idsqs.d;
                iqs_var.ffw = RS*idsqs.q + omega_elt*lambda_OBS.amp;
                // iqs_var.ffw = RS*idsqs.q + (omega_elt + (delta - delta_old)*fs)*lambda_OBS.amp;
                // delta_old = delta;
                ...
                //-----------------iqs Control Loop----------------------//
                iqs_par.lim = sqrt(pow(vdc*SQRT1OVER3,2)-pow(vdsqs_ref.d,2));
                iqs_var.ref = iqs_ref;
                iqs_var.fbk = idsqs.q;
			...
			if(SS_on){//-----------HF injection--------------------------//
				if (fabsf(omega_elt) < KOBS + 1.2*OMEGA_G && SS_on == 1) {
					switch(inj_waveform) {
						case 0: // pulsating
    ...
    if (duty_abc.a > 1.0f) duty_abc.a=1.0f;
    if (duty_abc.b > 1.0f) duty_abc.b=1.0f;
    if (duty_abc.c > 1.0f) duty_abc.c=1.0f;
    
    if (duty_abc.a < 0.0f) duty_abc.a=0.0f;
    if (duty_abc.b < 0.0f) duty_abc.b=0.0f;
    if (duty_abc.c < 0.0f) duty_abc.c=0.0f;
