...
if strcmp(geo.RotType,'Hybrid')
    [R2x_, R2y_] = drawHybridRotorCore(geo, sketch_rotor, geomApp);
else
    ...
    if (geo.p < 5 )
        for i = 2:length(AirRotorArea(:,1))
           ...
           for i = 2:(length(AirRotorArea(:,1))-1)
            regionName = ['Region.', num2str(i)];
            rotor_regiondel = sketch_RegionItem.GetItem(regionName);
             % Check if the region exists before attempting to delete
             if ~isempty(rotor_regiondel)
...
Protor = JDesigner.CreatePoint(geo.BLKLABELS.rotore.xy(rowIndex,1), geo.BLKLABELS.rotore.xy(rowIndex,2), 0);%should be modified
...
% '########################### Magnet group
if strcmp(geo.RotType,'Hybrid')
    MagnetCG=mean(geo.rotor(geo.rotor(:,end-1)==codMatBar,1:2));
    %MagnetCG=geo.BLKLABELS.rotore.xy(geo.BLKLABELS.rotore.xy(:,3)==codMatFeRot,1:2);%center of gravity(G) of Magnet
else
...
if(isnan(n3ph))
    ...
    if(n3ph == 1)
        ...
        for i = 1:1:n3ph
            % '########################### Create Groups for Coil U, W, and V
            model.GetGroupList().CreateGroup(strcat('Coil', '_U', num2str(i)));
            model.GetGroupList().CreateGroup(strcat('Coil', '_W', num2str(i)));
            model.GetGroupList().CreateGroup(strcat('Coil', '_V', num2str(i)));
        end
    end
end
...
if strcmp(geo.RotType,'Hybrid')
    ...
    RCoilCG=geo.BLKLABELS.rotore.xy(geo.BLKLABELS.rotore.xy(:,3)==codMatCuRot,1:2);%center of gravity(G) of Copper Coils
    RCoilCGX = RCoilCG(:,1);
    RCoilCGY = RCoilCG(:,2);
    ...
    model.GetGroupList().CreateGroup('RCoil');

    for slot = 1:2
        P = JDesigner.CreatePoint(RCoilCGX(slot),RCoilCGY(slot),0);
        rcoil = model.GetPartByPosition(P); 
        % coil_ID(slot) = coil.ID(); 
        rcoil.SetName(strcat('RCoil', num2str(rcoil.ID)));
        rcoil.SetColor('Green');
        model.GetGroupList().AddPartToGroup('RCoil',strcat('RCoil', num2str(rcoil.ID)));
    end          
end
...
if(isnan(n3ph))
    ...
    if(n3ph == 1)
        ...
        for i=1:1:n3ph
            % '#"Coil U" groups
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('U',num2str(i),'p'), +(3*i-2), 'green');
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('U',num2str(i),'n'), -(3*i-2), 'green');
            % '#"Coil V" groups
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('V',num2str(i),'p'), +(3*i-1), 'yellow');
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('V',num2str(i),'n'), -(3*i-1), 'yellow');
            % '#"Coil W" groups
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('W',num2str(i),'p'), +3*i, 'maroon');
            createCoilGroups_JMAG(model, JDesigner, CoilCGX_reshape, CoilCGY_reshape, geo, 'Coil', strcat('W',num2str(i),'n'), -3*i, 'maroon');
        end
    end
end
...
if strcmp(geo.RotType,'Hybrid')
    % '#"Rotor Coil" group
    createRCoilGroups_JMAG(model, JDesigner, RCoilCGX(1), RCoilCGY(1), geo, 'Coil','RCp', 'blue');
    createRCoilGroups_JMAG(model, JDesigner, RCoilCGX(2), RCoilCGY(2), geo, 'Coil','RCn', 'blue');
end
...
for ii=1:num_studies
    ...
    if(ii == 1)
        ...
        if strcmp(geo.RotType,'Hybrid')
            study_name = 'Demag';% '#study name
        else
            study_name = 'PWM';% '#study name
        end
    end
    ...
    if any(unique(Magnetxy(:,9)) ~= 0)
        if strcmp(geo.RotType,'Hybrid')
            ...
            PMTemp = mat.LayerMag.temp.temp(1,1);
        end
    end
    % % '------------------------------------------------------------------------
    % % 'Coil material
    ...
    property = material_properties_iron(deblank(stator_mat1));
    ...
    if(isnan(n3ph))
        ...
        if(n3ph==1)
            ...
            for i=1:1:n3ph
                % '#'Set up Coil material
                SetCoilMaterial_JMAG(study,coil_mat,eddycurrent_c,strcat('_U',num2str(i)))
                SetCoilMaterial_JMAG(study,coil_mat,eddycurrent_c,strcat('_V',num2str(i)))
                SetCoilMaterial_JMAG(study,coil_mat,eddycurrent_c,strcat('_W',num2str(i)))
            end
        end
    end
    
    if strcmp(geo.RotType,'Hybrid')
        study.SetMaterialByName('RCoil', coil_mat);
        study.GetMaterial('RCoil').SetValue('Laminated', 0);
        study.GetMaterial('RCoil').SetValue('EddyCurrentCalculation', eddycurrent_c);
        study.GetMaterial('RCoil').SetValue('UserConductivityType', 0);%'0=use material resistivity 1=Electric conductivity 2=Electric resistivity 3=Temperature dependant conductivity 4=Temperature dependant resistivity
    end
    ...
    if any(unique(Magnetxy(:,9)) ~= 0)
        if strcmp(geo.RotType,'Hybrid')
            MagnetCG=mean(geo.rotor(geo.rotor(:,end-1)==codMatBar,1:2));
            %MagnetCG=geo.BLKLABELS.rotore.xy(geo.BLKLABELS.rotore.xy(:,3)==codMatFeRot,1:2);%center of gravity(G) of Magnet
            MagnetizationDirection=geo.BLKLABELS.rotore.xy(geo.BLKLABELS.rotore.xy(:,3)==codMatAirRot,6:7);
        else
            MagnetCG=geo.BLKLABELS.rotore.xy(geo.BLKLABELS.rotore.xy(:,3)==codMatBar,1:2);%center of gravity(G) of Magnet
    ...
    JDesigner.SetCurrentStudy(study_name);
    if strcmp(geo.RotType,'Hybrid')
        pbcrx=(R1x+R2x_)/2.0; %'#rotor reference line
        pbcry=(R1y+R2y_)/2.0;
    else
